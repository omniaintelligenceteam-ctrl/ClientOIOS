...
function computeStats() {
  ...
  const totalDuration = demoCalls.reduce((sum, c) => sum + c.duration_seconds, 0)
  const avgDuration = Math.round(totalDuration / answered)
  const escalated = demoCalls.filter((c) => c.escalated_to_human).length
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Manage calls handled by your AI receptionist
          </p>
        </div>

        {/* Tabs */}
        <div className="flex items-center gap-1 rounded-xl bg-[#111827] p-1 border border-[rgba(148,163,184,0.1)]">
          <button className="rounded-lg bg-[#2DD4BF]/15 px-4 py-2 text-sm font-medium text-[#2DD4BF]">
            Log
          </button>
          <Link
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          ...
          <div
            key={s.label}
            className="rounded-2xl border border-[rgba(148,163,184,0.1)] bg-[#111827] p-4"
          >
              ...
              >
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium capitalize ${
                      statusColors[call.status] ?? statusColors.abandoned
                    }`}
                  >
                    {call.status}
                  </span>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="hidden text-xs capitalize text-slate-400 lg:inline">
                      {call.sentiment}
                    </span>
                  </div>
                </td>
                ...
                <td className="max-w-[260px] px-4 py-3">
                  <p className="truncate text-slate-400">
                    {call.transcript_summary ?? (
                      <span className="italic text-slate-600">No summary</span>
                    )}
                  </p>
                </td>
